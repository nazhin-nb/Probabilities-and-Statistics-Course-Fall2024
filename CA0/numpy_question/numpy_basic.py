...
def mutate_array(x, indices, values):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  for (i,j), k in zip(indices,values):
    x[i][j] = k
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def multiples_of_ten(start, stop):
  ...
  first = (start + 9) // 10 * 10
  if first <= stop:
    x = np.arange(first, stop + 1, 10, dtype=np.float64)
  else:
...
def zero_row_min(x):
  ...
  y[np.arange(x.shape[0]), indices] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  _, _, P = y.shape
  if use_loop:
    z = np.zeros((B, N, P), dtype=x.dtype)
    for i in range(B):
      z[i] = np.matmul(x[i], y[i])
  else:
    z = np.matmul(x, y)
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def normalize_columns(x):
  ...
  #############################################################################
  M, N = x.shape
  means = x.sum(axis=0) / M
  variances = ((x - means) ** 2).sum(axis=0) / M
